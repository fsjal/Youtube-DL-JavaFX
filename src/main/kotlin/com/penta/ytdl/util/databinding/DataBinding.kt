...
class DataBinding<T>(private val propertyName: String, private val widget: Any) : ReadWriteProperty<Any, T?> {
    ...
    override fun getValue(thisRef: Any, property: KProperty<*>) = getMethod(widget.javaClass)?.run {
        declaredMethods.find { it.name.contains(getPropertyName) && it.parameterCount == 0 }?.invoke(widget)
    } as T?

    override fun setValue(thisRef: Any, property: KProperty<*>, value: T?) {
        getMethod(widget.javaClass)?.run {
            declaredMethods.find { it.name.contains(setPropertyName) && it.parameterCount == 1 }?.invoke(widget, value)
        }
    }

    private fun getMethod(klass: Class<*>?): Class<*>? = klass?.run {
        if (declaredMethods.any { it.name.contains(getPropertyName) }) this else getMethod(superclass)
    }
}